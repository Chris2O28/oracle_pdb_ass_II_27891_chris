...
**PDB Name:** `Ch_pdb_27891`  
**Admin Username:** `Chris_plsqlauca_27891`  
...
CREATE PLUGGABLE DATABASE Ch_pdb_27891
    ADMIN USER Chris_plsqlauca_27891 IDENTIFIED BY 12345
    FILE_NAME_CONVERT = ('C:\APP\CHRIS\PRODUCT\21C\ORADATA\XE\PDBSEED\',
                         'C:\APP\CHRIS\PRODUCT\21C\ORADATA\XE\Ch_pdb_27891\');

-- Open the PDB
ALTER PLUGGABLE DATABASE Ch_pdb_27891 OPEN;

-- Verify PDB creation and status
SELECT name, open_mode FROM v$pdbs;

-- Switch to the PDB context
ALTER SESSION SET CONTAINER = Ch_pdb_27891;

-- Verify the admin user exists
SELECT username, account_status FROM dba_users WHERE username = 'CHRIS_PLSQLAUCA_27891';
```

### Results
- ✅ PDB `Ch_pdb_27891` created successfully
- ✅ PDB opened in READ WRITE mode
- ✅ Admin user `Chris_plsqlauca_27891` created and active
- ✅ User account ready for future class work

### Evidence
See screenshots folder: `task1_pdb_creation.png`, `task1_pdb_open.png`, `task1_user_created.png`
...
**Temporary PDB Name:** `CH_to_delete_pdb_27891`

### Commands Executed

```sql
-- Create temporary PDB
CREATE PLUGGABLE DATABASE CH_to_delete_pdb_27891
    ADMIN USER temp_admin IDENTIFIED BY temp123
    FILE_NAME_CONVERT = ('C:\APP\CHRIS\PRODUCT\21C\ORADATA\XE\PDBSEED\',
                         'C:\APP\CHRIS\PRODUCT\21C\ORADATA\XE\Ch_to_delete_pdb_27891\');

-- Open the temporary PDB
ALTER PLUGGABLE DATABASE CH_to_delete_pdb_27891 OPEN;

-- Verify it exists
SELECT name, open_mode FROM v$pdbs WHERE name = 'CH_TO_DELETE_PDB_27891';

-- Close the PDB before deletion
ALTER PLUGGABLE DATABASE CH_to_delete_pdb_27891 CLOSE IMMEDIATE;

-- Drop the PDB including datafiles
DROP PLUGGABLE DATABASE CH_to_delete_pdb_27891 INCLUDING DATAFILES;

-- Verify deletion (should return no rows)
SELECT name FROM v$pdbs WHERE name = 'CH_TO_DELETE_PDB_27891';
```
...
**Container Context:** Ch_pdb_27891
...
- PDB container information (Ch_pdb_27891)
...
I, **Chris (Student ID: 27891)**, hereby declare that:
